...
async def demo_sprint_planning():
    ...
    with Progress(
        ...
        team = [product_owner, scrum_master, tech_lead, frontend_dev, backend_dev, qa_engineer]
        
        # Phase 1: Product Owner Analysis
        task1 = progress.add_task("🎯 Product Owner analyzing requirements...", total=None)
        await product_owner.execute("Analyze product requirements and create user stories")
        ...
        progress.update(task2, description=f"✅ Created {len(user_stories)} user stories")
        
        # Phase 2: Tech Lead Analysis
        task3 = progress.add_task("🏗️  Tech Lead analyzing complexity...", total=None)
        await tech_lead.execute("Analyze technical complexity and break down into tasks")
        ...
        await scrum_master.execute("Create sprint plan with parallel execution strategy")
    ...
    parallelizable_tasks = len([t for t in tasks if t.task_type in ["frontend", "backend"]])
    ...
    metrics_table.add_row("Parallelizable Tasks", f"{parallelizable_tasks} ({parallelizable_tasks/len(tasks)*100:.1f}%)")
...
async def main():
    ...
    try:
        ...
        console.print("   • Run full sprint planning: [bold]uv run gaggle sprint plan --goal 'Your project idea'[/bold]")
