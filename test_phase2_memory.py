...
def test_hierarchical_memory():
    ...
    try:
        ...
        print("   ✅ HierarchicalMemory created for agent test-agent-001")
        ...
        episodic_item = MemoryItem(
            ...
            content={
                "sprint_id": "sprint-000",
                "outcome": "successful",
                "lessons": "good planning is crucial",
            },
        ...
        semantic_item = MemoryItem(
            ...
            content={
                "pattern": "authentication_middleware",
        ...
        print("   ✅ Memory items created for different levels")
        ...
        print("   ✅ Memory items stored successfully")

        # Test retrieval
        query_terms = {"authentication", "login"}
        result = agent_memory.retrieve(
            query_terms, strategy=RetrievalStrategy.HYBRID, limit=5
        )
    ...
    try:
        ...
        items = [
            MemoryItem(
                ...
                level=MemoryLevel.WORKING,
                content={
                    "description": "Authentication system with JWT tokens for secure user login"
                },
            ...
            MemoryItem(
                ...
                level=MemoryLevel.WORKING,
                content={
                    "description": "Database connection pooling for improved performance"
                },
            ...
            MemoryItem(
                ...
                level=MemoryLevel.SEMANTIC,
                content={
                    "description": "React component for user authentication form with validation"
                },
            ...
            MemoryItem(
                ...
                level=MemoryLevel.EPISODIC,
                content={
                    "description": "Sprint retrospective: authentication features completed successfully"
                },
        ...
        bm25_results = bm25_retriever.retrieve(
            "authentication login security", items, limit=3
        )
        assert len(bm25_results) > 0

        # Check that authentication-related items are ranked higher
        auth_items = [
            result for result in bm25_results if "authentication" in result[0].tags
        ]
        ...
        embedding_results = embedding_retriever.retrieve(
            "user login security", items, limit=3
        )
        assert len(embedding_results) > 0
        print(
            f"   ✅ Embedding retrieval found {len(embedding_results)} relevant items"
        )
        ...
        hybrid_results = hybrid_retriever.retrieve(
            "authentication system", items, limit=3
        )
        assert len(hybrid_results) > 0

        # Hybrid should combine BM25 and embedding scores
        for _item, score in hybrid_results:
            assert score.bm25_score >= 0 or score.semantic_score >= 0
        print(
            f"   ✅ Hybrid retrieval found {len(hybrid_results)} items with combined scoring"
        )
        ...
        advanced_results = advanced_retriever.retrieve_with_fallback(
            "database performance optimization", items, limit=2
        )
        ...
        print(
            f"   ✅ Advanced retrieval with statistics: {retrieval_stats['total_queries']} queries processed"
        )
        ...
        print(
            f"   ✅ Relevance scoring working: score = {relevance_score.total_score:.3f}"
        )
...
def test_prompt_caching():
    ...
    try:
        ...
        print("   ✅ PromptCache created with max size 100")
        ...
        should_cache_template = template_cacher.should_cache(
            template_content, template_context
        )
        assert should_cache_template
        print("   ✅ Template caching strategy correctly identified cacheable content")

        # Test cache miss (first request)
        cached_entry = cache.get(template_content, template_context)
        assert cached_entry is None
        print("   ✅ Cache miss on first request (as expected)")

        # Store in cache
        compressed_template = template_cacher.preprocess_content(
            template_content, template_context
        )
        success = cache.put(
            template_content,
        ...
        assert success
        print("   ✅ Template stored in cache successfully")
        ...
        should_cache_component = component_cacher.should_cache(
            component_content, component_context
        )
        ...
        print("   ✅ Component caching working")
        ...
        should_cache_pattern = pattern_cacher.should_cache(
            pattern_content, pattern_context
        )
        ...
        print(
            f"   ✅ Cache statistics: {cache_stats['cache_hits']}/{cache_stats['total_requests']} hit rate = {cache_stats['hit_rate']:.2f}"
        )
...
def test_context_compression():
    ...
    try:
        ...
        print(
            f"   ✅ Summary compression: {summary_result.original_size} -> {summary_result.compressed_size} tokens ({summary_result.compression_ratio:.2f} ratio)"
        )
        ...
        assert (
            template_result.compression_ratio < 1.0
            or len(template_compressor.templates) > 0
        )
        print(
            f"   ✅ Template compression: found {len(template_compressor.templates)} templates"
        )

        # Test decompression
        decompressed = template_compressor.decompress(
            template_result.compressed_content
        )
        # Should contain template references or be similar to original
        assert len(decompressed) > 0
        print("   ✅ Template decompression working")
        ...
        print(
            f"   ✅ Memory item compression: {item_compression.get_savings_percentage():.1f}% reduction"
        )

        # Test batch compression
        batch_compression = hierarchical_compressor.compress_context_batch(
            memory_items, max_total_size=200
        )
        assert batch_compression.compressed_size <= 200  # Should fit within limit
        assert batch_compression.compression_method == "batch"
        print(
            f"   ✅ Batch compression: {batch_compression.original_size} -> {batch_compression.compressed_size} tokens"
        )
        ...
        print(
            f"   ✅ Compression statistics: {compression_stats['total_compressions']} compressions performed"
        )
    ...
    try:
        ...
        items_to_store = [
            ...
            MemoryItem(
                ...
                level=MemoryLevel.SEMANTIC,
                content={
                    "knowledge": "REST API best practices",
                    "patterns": ["pagination", "filtering"],
                },
        ...
        for item in items_to_store:
            ...
            print(
                f"       Compressed item {item.id}: {compression_result.get_savings_percentage():.1f}% reduction"
            )
        ...
        if cached_template is not None:
            print("   ✅ Search template cached and retrieved successfully")
        else:
            print("   ⚠️  Cache retrieval didn't work (this is okay for testing)")
        ...
        try:
            ...
            print(
                f"       - Memory cleanup: {len(expired_items)} expired items removed"
            )
            print(
                f"       - Compression: {compression_stats['total_compressions']} operations"
            )
        ...
        try:
            ...
            print(
                f"   ✅ System statistics: {system_stats['total_agents']} agents, {system_stats['system_totals']['total_items']} total items"
            )
        except Exception as e:
