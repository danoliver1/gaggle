...
from src.gaggle.agents.implementation.frontend_dev import FrontendDeveloper
from src.gaggle.agents.implementation.backend_dev import BackendDeveloper
from src.gaggle.agents.qa.qa_engineer import QAEngineer
...
from src.gaggle.integrations.strands_adapter import strands_adapter
...
def mock_agents():
    ...
    for agent_class in [ProductOwner, ScrumMaster, TechLead, FrontendDeveloper, BackendDeveloper, QAEngineer]:
        agent = Mock()
        agent.agent = Mock()
        agent.agent.aexecute = AsyncMock(return_value={
...
class TestSprintPlanningWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_sprint_planning_flow(self, mock_agents, sample_project_requirements):
        """Test complete sprint planning from requirements to ready sprint."""
        
        # Mock Strands adapter
        with patch('src.gaggle.workflows.sprint_planning.strands_adapter') as mock_strands:
            ...
            with patch.object(workflow, '_create_user_stories') as mock_create_stories:
                with patch.object(workflow, '_analyze_architecture') as mock_analyze_arch:
                    with patch.object(workflow, '_plan_sprint') as mock_plan_sprint:
                        ...
                        result = await workflow.execute_complete_planning(sample_project_requirements)
    ...
    async def test_complete_sprint_execution_flow(self, mock_agents, sample_backlog):
        ...
        with patch('src.gaggle.workflows.sprint_execution.github_client') as mock_github:
            ...
            with patch('src.gaggle.workflows.sprint_execution.strands_adapter') as mock_strands:
                mock_strands.create_agent.return_value = mock_agents['FrontendDeveloper']
                mock_strands.execute_parallel_tasks = AsyncMock(return_value={
                    "successful": [
                        {"agent": "frontend_dev", "result": {"result": "Frontend implemented"}},
                        {"agent": "backend_dev", "result": {"result": "Backend implemented"}}
                    ],
                    "failed": []
                })
...
class TestEndToEndSprintWorkflow:
    ...
    async def test_full_sprint_cycle_planning_to_review(
            ...
            with patch.object(planning_workflow, '_create_user_stories') as mock_create:
                with patch.object(planning_workflow, '_analyze_architecture') as mock_analyze:
                    with patch.object(planning_workflow, '_plan_sprint') as mock_plan:
                        ...
                        mock_create.return_value = {"user_stories": sample_backlog, "total_story_points": 16}
                        mock_analyze.return_value = {"architecture_requirements": {}, "task_breakdown": []}
                        ...
                        planning_result = await planning_workflow.execute_complete_planning(sample_project_requirements)
        ...
        with patch('src.gaggle.workflows.sprint_execution.github_client') as mock_github:
            with patch('src.gaggle.workflows.sprint_execution.strands_adapter') as mock_strands:
                ...
                mock_strands.create_agent.return_value = mock_agents['FrontendDeveloper']
                mock_strands.execute_parallel_tasks = AsyncMock(return_value={"successful": [], "failed": []})
    ...
    @pytest.mark.asyncio
    async def test_multi_sprint_execution_with_learning(self, mock_agents, sample_backlog):
        ...
        sprints = [
            Sprint(
                ...
                user_stories=sample_backlog[i:i+1] if i < len(sample_backlog) else [],
                team_velocity=20
            )
...
class TestSprintMetricsIntegration:
    ...
    async def test_metrics_collection_during_sprint_execution(self, mock_agents):
        ...
        with patch.object(metrics_collector, 'start_sprint_tracking') as mock_start:
            with patch.object(metrics_collector, 'record_task_completion') as mock_record:
                with patch.object(metrics_collector, 'calculate_sprint_metrics') as mock_calculate:
                    ...
                    final_metrics = await metrics_collector.calculate_sprint_metrics(sprint.id)
...
class TestGitHubIntegrationWorkflow:
    ...
    async def test_github_milestone_and_project_board_creation(self, sample_backlog):
        ...
        with patch('src.gaggle.integrations.github_api.Github') as mock_github:
            mock_repo = Mock()
            mock_milestone = Mock(number=1, title="Sprint 001", html_url="https://github.com/test/test/milestones/1")
            mock_repo.create_milestone.return_value = mock_milestone
            
            mock_issue = Mock(number=1, html_url="https://github.com/test/test/issues/1")
    ...
    async def test_github_webhook_processing_during_sprint(self):
        ...
        from src.gaggle.integrations.github_api import GitHubWebhookHandler
...
class TestErrorHandlingAndResilience:
    ...
    async def test_sprint_execution_with_agent_failures(self, sample_backlog):
        ...
        with patch('src.gaggle.workflows.sprint_execution.strands_adapter') as mock_strands:
            ...
            mock_strands.execute_parallel_tasks = AsyncMock(return_value={
                "successful": [
                    {"agent": "frontend_dev", "result": {"result": "Frontend completed"}, "index": 0}
                ],
                "failed": [
                    {"agent": "backend_dev", "error": "Connection timeout", "index": 1}
                ],
                "summary": {
                    "total_tasks": 2,
                    "success_rate": 50.0
