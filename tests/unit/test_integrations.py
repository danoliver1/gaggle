...
from src.gaggle.integrations.github_api import GitHubAPIClient, GitHubWebhookHandler
from src.gaggle.integrations.strands_adapter import StrandsFrameworkAdapter, MockStrandsAgent
from src.gaggle.integrations.llm_providers import LLMProviderManager, AnthropicProvider, AWSBedrockProvider
from src.gaggle.integrations.cicd_pipelines import CICDPipelineManager, GitHubActionsPipeline, PipelineProvider
from src.gaggle.config.models import ModelTier
from src.gaggle.models.sprint import Sprint, UserStory
...
class TestGitHubAPIClient:
    ...
    async def test_sync_sprint_with_github(self, sample_sprint, mock_github_client):
        ...
        with patch('src.gaggle.integrations.github_api.Github', return_value=mock_github_client):
            client = GitHubAPIClient("test-token", "test-org", "test-repo")
...
class TestStrandsFrameworkAdapter:
    ...
    def test_init_with_strands_unavailable(self):
        ...
        with patch('src.gaggle.integrations.strands_adapter.Agent', side_effect=ImportError):
    ...
    def test_create_agent_with_mock(self):
        ...
        with patch('src.gaggle.integrations.strands_adapter.Agent', side_effect=ImportError):
            ...
            from src.gaggle.config.models import AgentRole
...
class TestMockStrandsAgent:
    ...
    async def test_execute_with_real_llm_tier(self):
        ...
        with patch('src.gaggle.integrations.strands_adapter.llm_provider_manager') as mock_provider:
            mock_provider.generate_response = AsyncMock(return_value={
...
class TestLLMProviderManager:
    ...
    async def test_generate_response_anthropic(self):
        ...
        with patch.object(manager.anthropic_provider, 'generate_response') as mock_generate:
...
class TestAnthropicProvider:
    ...
    async def test_generate_response(self):
        ...
        with patch('src.gaggle.integrations.llm_providers.anthropic.AsyncAnthropic') as mock_anthropic:
            mock_client = Mock()
            mock_response = Mock()
...
class TestCICDPipelineManager:
    ...
    def test_configure_pipeline(self):
        ...
        from src.gaggle.integrations.cicd_pipelines import PipelineConfiguration, DeploymentEnvironment
    ...
    async def test_setup_sprint_pipeline(self, sample_sprint):
        ...
        with patch.object(manager.github_actions, 'create_sprint_workflow') as mock_create:
...
class TestGitHubActionsPipeline:
    ...
    async def test_trigger_deployment(self, sample_sprint):
        ...
        from src.gaggle.integrations.cicd_pipelines import DeploymentEnvironment
...
class TestIntegrationFlow:
    ...
    async def test_github_strands_integration(self, sample_sprint):
        ...
        with patch('src.gaggle.integrations.github_api.Github') as mock_github:
            with patch('src.gaggle.integrations.strands_adapter.Agent', side_effect=ImportError):
                ...
                # Test workflow
                github_result = await github_client.sync_sprint_with_github(sample_sprint)
                
                agents = []
                tasks = ["analyze requirements", "create implementation plan"]
                
                strands_result = await strands_adapter.execute_parallel_tasks(agents, tasks)
    ...
    async def test_llm_cicd_integration(self, sample_sprint):
        ...
        # Test workflow
        with patch.object(llm_manager.anthropic_provider, 'generate_response') as mock_llm:
            mock_llm.return_value = {
                "response": "Pipeline setup complete",
                "usage": {"input_tokens": 100, "output_tokens": 200}
            }
            
            with patch.object(cicd_manager.github_actions, 'create_sprint_workflow') as mock_workflow:
