...
from src.gaggle.models.story import UserStory, StoryStatus
...
class TestProductOwner:
    ...
    async def test_analyze_requirements(self, product_owner, mock_anthropic_response):
        ...
        with patch.object(product_owner, 'execute', return_value=mock_anthropic_response):
            
            product_idea = "Build a user management system with authentication and profiles"
    ...
    async def test_create_user_stories(self, product_owner, mock_anthropic_response):
        ...
        with patch.object(product_owner, 'execute', return_value=mock_anthropic_response):
            ...
            stories = await product_owner.create_user_stories(product_idea, clarifications)
    ...
    @pytest.mark.asyncio
    async def test_prioritize_backlog(self, product_owner, sample_user_stories, mock_anthropic_response):
        """Test backlog prioritization functionality."""
        
        with patch.object(product_owner, 'execute', return_value=mock_anthropic_response):
            
            prioritized_stories = await product_owner.prioritize_backlog(sample_user_stories)
    ...
    @pytest.mark.asyncio
    async def test_review_sprint_deliverables(self, product_owner, sample_user_stories, mock_anthropic_response):
        ...
        with patch.object(product_owner, 'execute', return_value=mock_anthropic_response):
            ...
            review_result = await product_owner.review_sprint_deliverables(sprint, sample_user_stories)
