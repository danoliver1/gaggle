...
from src.gaggle.agents.implementation.fullstack_dev import FullstackDeveloper
...
def mock_strands_agent():
    ...
    agent = Mock()
    agent.aexecute = AsyncMock(return_value={
...
class TestTechLead:
    ...
    @pytest.mark.asyncio
    async def test_analyze_architecture_requirements(self, mock_strands_agent, sample_user_story):
        ...
        result = await tl.analyze_architecture_requirements(user_stories, project_context)
...
class TestQAEngineer:
    ...
    async def test_execute_functional_testing(self, mock_strands_agent, sample_task):
        ...
        test_scenarios = [
            {"name": "Valid login", "steps": ["Enter valid credentials", "Click login"]},
            {"name": "Invalid login", "steps": ["Enter invalid credentials", "Verify error"]}
        ]
...
class TestAgentIntegration:
    ...
    async def test_story_creation_and_breakdown_flow(self, mock_strands_agent):
        ...
        arch_result = await tl.analyze_architecture_requirements(user_stories, project_context)
