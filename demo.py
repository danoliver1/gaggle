...
async def demo_sprint_planning():
    ...
    # Create sprint model
    sprint = SprintModel(
        id="SPRINT-001", goal=sprint_plan["sprint_goal"], status=SprintStatus.PLANNING
    )
    ...
    for story_id, tasks in tasks_by_story.items():
        story_title = next(
            (s.title for s in prioritized_stories if s.id == story_id), "Unknown"
        )
        for task in tasks:
            tasks_table.add_row(
                task.title,
    ...
    metrics_table.add_row(
        ...
        f"{parallelizable_tasks} ({parallelizable_tasks/total_tasks*100:.1f}%)",
    )
    metrics_table.add_row(
    ...
    for member in team_config.members:
        specializations = ", ".join(member.specializations[:2]) + (
            "..." if len(member.specializations) > 2 else ""
        )
        team_table.add_row(
            member.name,
            member.role.value,
    ...
    console.print(
        Panel(
            f"""[bold]🏗️  Architecture Highlights:[/bold]

• [green]Token Efficiency:[/green] Reusable components save ~{total_estimated_savings:,} tokens across sprint
• [blue]Parallelization:[/blue] {int(parallelizable_tasks/total_tasks*100):d}% of tasks can run simultaneously
...
4. Perform code reviews and integration testing""",
...
async def main():
    ...
    try:
        ...
        console.print(
            "🔗 Explore the codebase at: https://github.com/danoliver1/gaggle"
        )
