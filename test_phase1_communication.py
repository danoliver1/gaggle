...
def test_message_schema_creation():
    ...
    try:
        ...
        task_message = TaskAssignmentMessage(
            ...
            estimated_effort=5,
            acceptance_criteria=["Form validates input", "Shows error messages", "Responsive design"]
        )
        
        validation = task_message.validate()
        assert validation.is_valid, f"Task message validation failed: {validation.errors}"
        print(f"   ✅ TaskAssignmentMessage created and validated")
        ...
        assert validation.is_valid, f"Planning message validation failed: {validation.errors}"
        print(f"   ✅ SprintPlanningMessage created and validated")
        ...
        assert validation.is_valid, f"Standup message validation failed: {validation.errors}"
        print(f"   ✅ StandupUpdateMessage created and validated")
...
def test_state_machine_creation():
    ...
    try:
        ...
        print(f"   ✅ ProductOwner state machine created (state: {po_sm.current_state.value})")
        ...
        print(f"   ✅ State transition successful: IDLE -> PLANNING")
        
        # Test capabilities
        capabilities = po_sm.get_capabilities_for_state(AgentState.PLANNING)
        assert "create_user_stories" in capabilities
        print(f"   ✅ Planning state capabilities: {len(capabilities)} actions available")
        ...
        print(f"   ✅ TechLead state machine created")
        ...
        print(f"   ✅ Developer state machine created")
...
def test_context_management():
    ...
    try:
        from gaggle.core.state.context import (
            ContextManager,
            ContextItem,
            ContextLevel
        )
        ...
        print(f"   ✅ Context item stored and retrieved successfully")
...
async def test_message_bus():
    ...
    try:
        ...
        from gaggle.core.communication.messages import TaskAssignmentMessage, MessageType
        from gaggle.config.models import AgentRole
        from gaggle.models.task import TaskType
        ...
        print(f"   ✅ Message bus metrics: {metrics['total_messages']} messages processed")
        
        await message_bus.stop()
        print(f"   ✅ Message bus stopped")
...
async def test_protocol_validation():
    ...
    try:
        ...
        print(f"   ✅ Valid message passed protocol validation")
        ...
        assert len(status) > 0
        print(f"   ✅ Protocol status retrieved")
