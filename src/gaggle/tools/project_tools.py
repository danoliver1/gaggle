...
class BacklogTool(BaseTool):
    ...
    async def prioritize_stories(self, stories: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "rationale": "Prioritized based on business value, story size, dependencies, and user impact",
        }
    ...
    async def estimate_story_points(self, story: dict) -> dict[str, Any]:
        ...
        if any(
            word in description for word in ["complex", "integration", "architecture"]
        ):
            points = 8
        ...
        return {
            ...
            "confidence": "medium",
            "rationale": "Based on complexity indicators in story description",
        }
    ...
    async def validate_story(self, story: dict) -> dict[str, Any]:
        ...
        if not any(
            phrase in description.lower() for phrase in ["as a", "i want", "so that"]
        ):
    ...
    def _get_improvement_suggestions(self, issues: list[str]) -> list[str]:
        ...
        for issue in issues:
            if "format" in issue:
                ...
                suggestions.append(
                    "Estimate complexity using planning poker (1,2,3,5,8,13)"
                )
...
class StoryTemplateTool(BaseTool):
    ...
    async def execute(
        self, template_type: str = "standard", **kwargs
    ) -> dict[str, Any]:
...
class SprintBoardTool(BaseTool):
    ...
    async def execute(self, action: str, **kwargs) -> dict[str, Any]:
        ...
        if action == "update_status":
            return await self.update_item_status(
                kwargs.get("item_id"), kwargs.get("new_status")
            )
        elif action == "get_board_state":
            return await self.get_board_state(kwargs.get("sprint_id"))
        elif action == "move_item":
            return await self.move_item(
                kwargs.get("item_id"),
                kwargs.get("from_column"),
    ...
    async def move_item(
        self, item_id: str, from_column: str, to_column: str
    ) -> dict[str, Any]:
...
class MetricsTool(BaseTool):
    ...
    async def get_team_performance(self, team_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "areas_for_improvement": ["Estimation accuracy", "Early testing"],
        }
