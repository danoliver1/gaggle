...
class DailyStandupWorkflow:
    ...
    def __init__(self, sprint: SprintModel, context: AgentContext | None = None):
        self.sprint = sprint
    ...
    async def facilitate_standup(
        self, day: int, team_updates: list[dict[str, Any]] = None
    ) -> dict[str, Any]:
    ...
    def _gather_sprint_status(self) -> dict[str, Any]:
        ...
        completed_tasks = len(
            [t for t in self.sprint.tasks if t.status == TaskStatus.COMPLETED]
        )
        in_progress_tasks = len(
            [t for t in self.sprint.tasks if t.status == TaskStatus.IN_PROGRESS]
        )
        blocked_tasks = len(
            [t for t in self.sprint.tasks if t.status == TaskStatus.BLOCKED]
        )
    ...
    def _generate_team_updates(self) -> list[dict[str, Any]]:
        ...
        fullstack_tasks = [
            t
            for t in self.sprint.tasks
            if "integration" in t.description.lower()
            or "feature" in t.description.lower()
        ]
        if fullstack_tasks:
            team_updates.append(
                {
    ...
    async def _conduct_standup_discussion(
        ...
        return {
            ...
            "action_items": [
                {
                    "action": "Resolve test environment setup",
                    "owner": "DevOps",
                    "due": "today",
                },
                {
                    "action": "Review API integration dependencies",
                    "owner": "Tech Lead",
                    "due": "today",
                },
            ],
    ...
    def _format_team_updates_for_prompt(
        self, team_updates: list[dict[str, Any]]
    ) -> str:
...
"""
    ...
    def _calculate_team_confidence(self, team_updates: list[dict[str, Any]]) -> float:
        ...
        confidence_levels = [
            update.get("confidence_level", 8.0) for update in team_updates
        ]
        return sum(confidence_levels) / len(confidence_levels)

    async def _plan_blocker_resolution(
        self, team_updates: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def _update_sprint_tracking(
        ...
        team_confidence = self._calculate_team_confidence(team_updates)
        blockers_count = len(
            [u for u in team_updates if u["blockers"] != "No blockers"]
        )
        ...
        tracking_update = {
            ...
            "sprint_health": (
                "green"
                if team_confidence >= 8.5 and blockers_count == 0
                else (
    ...
    def _calculate_days_elapsed(self) -> int:
        ...
        return max(
            0, min(days_elapsed, int(self.sprint.duration_weeks * 5))
        )  # Cap at sprint duration
