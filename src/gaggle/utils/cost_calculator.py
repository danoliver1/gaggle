...
from ..config.models import AgentRole, ModelTier, calculate_cost, get_model_config
...
class CostCalculator:
    ...
    def __init__(self):
        self.model_configs = {role: get_model_config(role) for role in AgentRole}

    def estimate_task_cost(self, task: TaskEstimate, agent_role: AgentRole) -> float:
        """Estimate cost for a task with a specific agent role."""
        config = self.model_configs[agent_role]
        return calculate_cost(
            task.estimated_input_tokens, task.estimated_output_tokens, config
        )

    def estimate_task_time(self, task: TaskEstimate, agent_role: AgentRole) -> float:
    ...
    def optimize_allocation(
        ...
        for task in sorted_tasks:
            best_allocation = self._find_best_allocation(task, allocations, strategy)
    ...
    def _sort_tasks_by_priority(self, tasks: list[TaskEstimate]) -> list[TaskEstimate]:
        ...
        return sorted(
            tasks,
            key=lambda t: (dependency_depth(t), -complexity_order.get(t.complexity, 1)),
        )
    ...
    def _find_best_allocation(
        ...
        if strategy == OptimizationStrategy.MINIMIZE_COST:
            ...
            return min(
                suitable_allocations,
                key=lambda a: a.estimated_time
                + self.estimate_task_time(task, a.agent_role),
            )
            ...
            def balanced_score(allocation: AgentAllocation) -> float:
                ...
                time = allocation.estimated_time + self.estimate_task_time(
                    task, allocation.agent_role
                )
    ...
    def _filter_suitable_allocations(
        self, task: TaskEstimate, allocations: list[AgentAllocation]
    ) -> list[AgentAllocation]:
        ...
        return [alloc for alloc in allocations if alloc.agent_role in suitable_roles]

    def calculate_sprint_metrics(
        self, allocations: list[AgentAllocation]
    ) -> dict[str, float]:
        ...
        max_time = max((alloc.estimated_time for alloc in allocations), default=0.0)
