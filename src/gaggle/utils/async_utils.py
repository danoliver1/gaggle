...
async def run_with_timeout(
    awaitable: Awaitable[T], timeout_seconds: float, default_value: T | None = None
) -> T:
    """
...
async def throttled_gather(
    ...
    wrapped_awaitables = [
        _throttled_execution(i, aw) for i, aw in enumerate(awaitables)
    ]
...
class ParallelExecutor:
    ...
    def __init__(self, max_concurrency: int = 5, rate_limit: float | None = None):
        ...
        try:
            if strategy == "sequential":
                ...
                results = await gather_with_concurrency(
                    awaitables, self.max_concurrency
                )
