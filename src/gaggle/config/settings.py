...
class GaggleSettings(BaseSettings):
    ...
    aws_profile: Optional[str] = Field(None, description="AWS profile for Bedrock")
    ...
    default_sprint_duration: int = Field(10, description="Default sprint duration in days")
    ...
    sonnet_model: str = Field("claude-3-5-sonnet-20241022", description="Sonnet model ID")
    opus_model: str = Field("claude-3-opus-20240229", description="Opus model ID")
    
    # Cost Management
    token_budget_per_sprint: Optional[int] = Field(None, description="Token budget per sprint")
    cost_tracking_enabled: bool = Field(True, description="Enable cost tracking")
    max_cost_per_sprint: Optional[float] = Field(None, description="Maximum cost per sprint in USD")
