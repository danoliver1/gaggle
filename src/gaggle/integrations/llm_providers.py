...
class AnthropicProvider(BaseLLMProvider):
    ...
    async def generate_response(
        ...
        try:
            async with self.session.post(
                f"{self.base_url}/messages", json=request_data, headers=headers
            ) as response:
            ...
            self.logger.error(
                "anthropic_request_failed", model=self.config.model_id, error=str(e)
            )
            raise
    ...
    async def generate_streaming_response(
        ...
        try:
            ...
            self.logger.error(
                "anthropic_streaming_failed", model=self.config.model_id, error=str(e)
            )
            raise
...
class AWSBedrockProvider(BaseLLMProvider):
    ...
    def __init__(self, config: ModelConfig, region: str = "us-east-1"):
        ...
        try:
            ...
            self.logger.warning(
                "boto3_not_available", message="Using mock implementation"
            )
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(
                ...
                lambda: self.bedrock_client.invoke_model(
                    modelId=self.config.model_id, body=json.dumps(body)
                ),
            )
            ...
            self.logger.error(
                "bedrock_request_failed", model=self.config.model_id, error=str(e)
            )
            raise
...
class LLMProviderManager:
    ...
    def _initialize_providers(self):
        ...
        for tier, config in DEFAULT_MODEL_CONFIGS.items():
            try:
                ...
                self.logger.error(
                    "llm_provider_init_failed", tier=tier.value, error=str(e)
                )

    async def generate_response(
        self, tier: ModelTier, prompt: str, system_prompt: str | None = None, **kwargs
    ) -> dict[str, Any]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(
                    {"error": str(result), "request_index": i, "status": "failed"}
                )
            else:
    ...
    def get_all_usage_metrics(self) -> dict[str, Any]:
        ...
        metrics = {"total_cost": 0.0, "total_tokens": 0, "providers": {}}
        ...
        metrics["cost_by_tier"] = {
            tier.value: provider.total_cost for tier, provider in self.providers.items()
        }
