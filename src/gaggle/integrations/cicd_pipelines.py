...
class GitHubActionsPipeline:
    ...
    async def create_sprint_workflow(self, sprint: Sprint) -> str:
        ...
        self._generate_sprint_workflow_yaml(sprint)
    ...
    def _generate_sprint_workflow_yaml(self, sprint: Sprint) -> str:
        ...
        workflow = {
            ...
            "on": {
                "push": {"branches": [f"sprint/{sprint.id}", "main"]},
    ...
    async def _simulate_deployment(
        self, execution: PipelineExecution, artifacts: dict[str, str]
    ):
        """Simulate deployment process."""
...
class CICDPipelineManager:
    ...
    def configure_pipeline(self, project_id: str, configuration: PipelineConfiguration):
    ...
    async def setup_sprint_pipeline(self, sprint: Sprint) -> dict[str, Any]:
        ...
        if not config:
            raise ValueError(
                f"No pipeline configuration found for project {project_id}"
            )
        ...
        for environment in config.environments:
            ...
            setup_results["environments_configured"].append(
                {"environment": environment.value, "configuration": env_config}
            )
    ...
    async def _configure_environment(
        self, environment: DeploymentEnvironment, sprint: Sprint
    ) -> dict[str, Any]:
    ...
    async def execute_deployment_pipeline(
        ...
        if config.provider == PipelineProvider.GITHUB_ACTIONS:
            execution = await self.github_actions.trigger_deployment(
                environment=environment, sprint_id=sprint.id, artifacts=artifacts
            )
        else:
    ...
    async def _prepare_deployment_artifacts(
        self, sprint: Sprint, user_story_id: str | None = None
    ) -> dict[str, str]:
    ...
    async def monitor_pipeline_execution(self, pipeline_id: str) -> dict[str, Any]:
    ...
    async def handle_sprint_completion(self, sprint: Sprint) -> list[PipelineExecution]:
        ...
        if DeploymentEnvironment.STAGING in config.environments:
            staging_execution = await self.execute_deployment_pipeline(
                sprint=sprint, environment=DeploymentEnvironment.STAGING
            )
            ...
            while staging_execution.status == PipelineStatus.RUNNING:
                ...
                monitoring_data = await self.monitor_pipeline_execution(
                    staging_execution.pipeline_id
                )
                staging_execution.status = PipelineStatus(monitoring_data["status"])

        # Deploy to production if staging succeeded
        if (
            DeploymentEnvironment.PRODUCTION in config.environments
            and staging_execution.status == PipelineStatus.SUCCESS
        ):

            production_execution = await self.execute_deployment_pipeline(
                sprint=sprint, environment=DeploymentEnvironment.PRODUCTION
            )
    ...
    async def get_pipeline_metrics(
        ...
        successful_executions = len(
            [e for e in relevant_executions if e.status == PipelineStatus.SUCCESS]
        )
        failed_executions = len(
            [e for e in relevant_executions if e.status == PipelineStatus.FAILURE]
        )
            ...
            "by_status": {
                ...
                "pending": len(
                    [
                        ...
                        if e.status == PipelineStatus.PENDING
                    ]
                ),
                "running": len(
                    [
        ...
        for env in DeploymentEnvironment:
            ...
            if env_executions:
                env_successful = len(
                    [e for e in env_executions if e.status == PipelineStatus.SUCCESS]
                )
                metrics["by_environment"][env.value] = {
                    "total": len(env_executions),
                    "successful": env_successful,
                    "success_rate_percent": (
                        env_successful / len(env_executions) * 100
                    ),
                }
