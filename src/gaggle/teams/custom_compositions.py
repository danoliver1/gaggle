...
class TeamCompositionBuilder:
    ...
    def _get_base_team_structure(
        ...
        base_agents.append(AgentSpecification(
            role=AgentRole.PRODUCT_OWNER,
            name="ProductOwner",
            model_tier=ModelTier.HAIKU,
            skills=["requirements_analysis", "stakeholder_management", "prioritization"],
        ...
        base_agents.append(AgentSpecification(
            ...
            name="ScrumMaster",
            model_tier=ModelTier.HAIKU,
            skills=["agile_facilitation", "team_coordination", "blocker_resolution"],
            capacity_percentage=100.0,
            cost_weight=1.0
        ))
    ...
    def _get_web_app_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
        """Get specialists for web application projects."""
        
        specialists = []
        
        # Frontend developers
        specialists.append(AgentSpecification(
        ...
        if requirements.team_size in [TeamSize.MEDIUM, TeamSize.LARGE, TeamSize.ENTERPRISE]:
            specialists.append(AgentSpecification(
        ...
        if requirements.team_size in [TeamSize.LARGE, TeamSize.ENTERPRISE]:
            specialists.append(AgentSpecification(
                ...
                model_tier=ModelTier.SONNET,
                skills=["performance_optimization", "bundle_optimization", "caching"],
    ...
    def _get_mobile_app_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
        ...
        specialists.append(AgentSpecification(
            ...
            model_tier=ModelTier.SONNET,
            skills=["mobile_apis", "push_notifications", "offline_sync", "security"],
            capacity_percentage=100.0,
            cost_weight=1.0
        ))
    ...
    def _get_api_service_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
    ...
    def _get_microservices_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
    ...
    def _get_data_pipeline_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
    ...
    def _get_ml_project_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
    ...
    def _get_devops_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
        ...
        specialists.append(AgentSpecification(
            ...
            model_tier=ModelTier.SONNET,
            skills=["infrastructure_as_code", "terraform", "ansible", "cloud_platforms"],
            capacity_percentage=100.0,
            cost_weight=1.0
        ))
        ...
        specialists.append(AgentSpecification(
            ...
            model_tier=ModelTier.SONNET,
            skills=["monitoring", "logging", "alerting", "performance_optimization"],
            capacity_percentage=100.0,
            cost_weight=1.0
        ))
    ...
    def _get_frontend_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
        ...
        specialists.append(AgentSpecification(
            ...
            model_tier=ModelTier.SONNET,
            skills=["react", "typescript", "state_management", "component_libraries"],
            capacity_percentage=100.0,
            cost_weight=1.0
        ))
        ...
        if requirements.team_size in [TeamSize.MEDIUM, TeamSize.LARGE, TeamSize.ENTERPRISE]:
            specialists.append(AgentSpecification(
                ...
                model_tier=ModelTier.SONNET,
                skills=["ux_design", "accessibility", "responsive_design", "animations"],
                capacity_percentage=100.0,
                cost_weight=1.0
            ))
    ...
    def _get_backend_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
    ...
    def _get_fullstack_specialists(self, requirements: TeamCompositionRequirements) -> List[AgentSpecification]:
    ...
    def _optimize_team_composition(
        ...
        if requirements.budget_constraints:
            agents = self._apply_budget_constraints(agents, requirements.budget_constraints)
    ...
    def _apply_budget_constraints(
        ...
        current_cost = sum(
            self.tier_costs[agent.model_tier] * agent.cost_weight 
            for agent in agents
        )
        ...
        essential_roles = {AgentRole.PRODUCT_OWNER, AgentRole.SCRUM_MASTER, AgentRole.TECH_LEAD}
        ...
        other_agents.sort(key=lambda a: len(a.skills or []) / self.tier_costs[a.model_tier], reverse=True)
        
        # Add agents while staying under budget
        optimized_agents = essential_agents.copy()
        remaining_budget = budget - sum(self.tier_costs[a.model_tier] for a in essential_agents)
    ...
    def _apply_team_size_constraints(
        ...
        agents_with_priority = [
            (agent, role_priorities.get(agent.role, 1) + (1 if agent.specialization else 0))
            for agent in agents
        ]
    ...
    def _skill_matches_role(self, skill: str, role: AgentRole) -> bool:
        ...
        frontend_skills = {"react", "typescript", "css", "html", "javascript", "responsive_design", "accessibility"}
        backend_skills = {"python", "fastapi", "postgresql", "redis", "docker", "api_development"}
        ...
        if role == AgentRole.FRONTEND_DEV and skill.lower() in frontend_skills:
            return True
        elif role == AgentRole.BACKEND_DEV and skill.lower() in backend_skills:
            return True
        elif role == AgentRole.FULLSTACK_DEV and skill.lower() in fullstack_skills:
            return True
        elif role == AgentRole.QA_ENGINEER and skill.lower() in qa_skills:
            return True
    ...
    def _finalize_composition(
        ...
        dev_agents = [a for a in agents if a.role in {AgentRole.FRONTEND_DEV, AgentRole.BACKEND_DEV, AgentRole.FULLSTACK_DEV}]
        estimated_velocity = len(dev_agents) * 8.0  # 8 story points per developer per sprint
        ...
        strengths, limitations = self._analyze_team_strengths_limitations(agents, requirements)
    ...
    def _analyze_team_strengths_limitations(
        ...
        specializations = {agent.specialization for agent in agents if agent.specialization}
        ...
        if requirements.project_type in {ProjectType.WEB_APPLICATION, ProjectType.MOBILE_APP}:
    ...
    def _generate_success_metrics(self, requirements: TeamCompositionRequirements) -> List[str]:
        ...
        if requirements.budget_constraints:
            base_metrics.append(f"Stay within budget of ${requirements.budget_constraints:.2f} per sprint")
        
        project_specific_metrics = {
            ProjectType.WEB_APPLICATION: ["Page load time <3 seconds", "Accessibility compliance 100%"],
            ProjectType.MOBILE_APP: ["App store rating >4.5", "Crash rate <1%"],
            ProjectType.API_SERVICE: ["API response time <500ms", "99.9% uptime"],
            ProjectType.MICROSERVICES: ["Service independence maintained", "Circuit breaker effectiveness >95%"],
            ProjectType.DATA_PIPELINE: ["Data quality >99.5%", "Pipeline reliability >99%"],
            ProjectType.ML_PROJECT: ["Model accuracy meets targets", "Model deployment success >98%"]
        }
...
class TeamCompositionManager:
    ...
    async def _generate_alternatives(
        ...
        if requirements.budget_constraints:
            budget_optimized_req = TeamCompositionRequirements(
                ...
                budget_constraints=requirements.budget_constraints * 0.8,  # 20% less budget
                timeline_weeks=requirements.timeline_weeks,
                specific_skills=requirements.specific_skills
            )
            budget_comp = await self.builder.build_team_composition(budget_optimized_req)
        ...
        perf_optimized_req = TeamCompositionRequirements(
            ...
            timeline_weeks=requirements.timeline_weeks,
            specific_skills=(requirements.specific_skills or []) + ["performance_optimization"]
        )
    ...
    def _compare_compositions(self, compositions: List[TeamComposition]) -> Dict[str, Any]:
        ...
        comparison = {
            "cost_comparison": {
                "lowest_cost": min(comp.estimated_cost_per_sprint for comp in compositions),
                "highest_cost": max(comp.estimated_cost_per_sprint for comp in compositions),
                "average_cost": sum(comp.estimated_cost_per_sprint for comp in compositions) / len(compositions)
            },
            "velocity_comparison": {
                "lowest_velocity": min(comp.estimated_velocity for comp in compositions),
                "highest_velocity": max(comp.estimated_velocity for comp in compositions),
                "average_velocity": sum(comp.estimated_velocity for comp in compositions) / len(compositions)
            },
            "team_size_comparison": {
                ...
                "average_team_size": sum(len(comp.agents) for comp in compositions) / len(compositions)
            },
            "value_analysis": []
        }
        ...
        for i, comp in enumerate(compositions):
            ...
            value_score = cost_efficiency * len(comp.strengths) / max(1, len(comp.limitations))
    ...
    def _generate_selection_guidance(
        ...
        for i, alt in enumerate(alternatives):
            if alt.estimated_cost_per_sprint < primary.estimated_cost_per_sprint:
                guidance["alternative_recommendations"][f"alternative_{i+1}"] = (
        ...
        if requirements.budget_constraints:
            within_budget = [comp for comp in [primary] + alternatives if comp.estimated_cost_per_sprint <= requirements.budget_constraints]
            if within_budget:
                best_in_budget = max(within_budget, key=lambda c: c.estimated_velocity)
                guidance["budget_constrained_choice"] = f"Best option within budget: {best_in_budget.name}"
        
        if requirements.timeline_weeks and requirements.timeline_weeks < 4:
            guidance["timeline_constrained_choice"] = "For short timelines, choose higher velocity options even if more expensive"
    ...
    def _composition_to_dict(self, composition: TeamComposition) -> Dict[str, Any]:
        ...
        return {
            ...
            "cost_per_story_point": composition.estimated_cost_per_sprint / max(1, composition.estimated_velocity)
        }