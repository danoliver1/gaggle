...
from .config.settings import settings, GaggleSettings
...
def init(
    ...
    github_token: str = typer.Option(..., "--token", envvar="GITHUB_TOKEN", help="GitHub personal access token"),
    anthropic_key: Optional[str] = typer.Option(None, "--anthropic-key", envvar="ANTHROPIC_API_KEY", help="Anthropic API key"),
    aws_profile: Optional[str] = typer.Option(None, "--aws-profile", envvar="AWS_PROFILE", help="AWS profile for Bedrock"),
):
    ...
    if not anthropic_key and not aws_profile:
        console.print("❌ Error: Either --anthropic-key or --aws-profile must be provided", style="red")
        raise typer.Exit(1)
...
def sprint(
    ...
    goal: Optional[str] = typer.Option(None, "--goal", help="Sprint goal or product idea"),
    ...
    if action == "plan":
        if not goal:
            console.print("❌ Error: --goal is required for sprint planning", style="red")
            raise typer.Exit(1)
...
async def plan_sprint(goal: str, duration: int):
    ...
    with Progress(
        ...
        try:
            ...
            analysis = await po.analyze_requirements(goal)
            ...
            task7 = progress.add_task("📅 Scrum Master creating sprint plan...", total=None)
    ...
    for story in prioritized_stories[:5]:  # Show first 5 stories
        complexity = complexity_analysis['complexity_scores'].get(story.id, {}).get('complexity_rating', 'Unknown')
    ...
    total_points = sum(story.story_points for story in prioritized_stories[:5])
    
    metrics_table.add_row("Estimated Velocity", f"{sprint_plan['estimated_velocity']} points")
    ...
    metrics_table.add_row("Estimated Token Savings", f"{reusable_components['total_estimated_savings']:,}")
...
@app.command()
def team(
    action: str = typer.Argument(..., help="Team action: status, add, remove, or configure"),
    member_name: Optional[str] = typer.Option(None, "--name", help="Team member name"),
...
def config(
    ...
    if show:
        ...
        config_table.add_row("Default Sprint Duration", f"{settings.default_sprint_duration} days")
        ...
        console.print(f"⚠️  Configuration updates not yet implemented", style="yellow")
    
    else:
        console.print("❌ Use --show to view config or --set with --value to update", style="red")
