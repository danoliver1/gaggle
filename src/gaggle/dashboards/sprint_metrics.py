...
class SprintMetricsCollector:
    ...
    def _calculate_progress_metrics(self, sprint: SprintModel) -> dict[str, Any]:
        ...
        completed_stories = len(
            [s for s in sprint.user_stories if s.status == StoryStatus.DONE]
        )
    ...
    def _calculate_efficiency_metrics(self, sprint: SprintModel) -> dict[str, Any]:
        ...
        automated_tasks = len(
            [
                ...
                or "automation" in t.description.lower()
            ]
        )
...
class SprintDashboard:
        ...
        dashboard = {
            ...
            "metrics": {
                ...
                "cost": {
                    ...
                    "efficiency": (
                        "High" if metrics.cost_per_story_point < 10.0 else "Medium"
                    ),
                },
            },
            "charts": charts,
        ...
        return {
            "progress_chart": {
                ...
                "data": {
                    ...
                    "color": (
                        "green"
                        if metrics.progress_percentage >= 80
                        else "yellow" if metrics.progress_percentage >= 50 else "red"
                    ),
                },
            },
            "velocity_chart": {
    ...
    async def _generate_insights(self, metrics: SprintMetrics) -> list[str]:
        ...
        # Velocity insight
        if metrics.velocity > metrics.planned_story_points / max(
            1, metrics.sprint_id.count("w")
        ):  # Mock calculation
            insights.append(
                f"🚀 Team velocity ({metrics.velocity:.1f} points/week) is exceeding expectations"
            )

        # Quality insight
        if metrics.test_coverage > 90:
            insights.append(
                f"✅ Excellent test coverage at {metrics.test_coverage:.1f}%"
            )

        # Efficiency insight
        if metrics.parallel_execution_rate > 80:
            insights.append(
                f"⚡ High parallel execution rate ({metrics.parallel_execution_rate:.1f}%) shows good task coordination"
            )

        # Cost insight
        if metrics.cost_per_story_point < 5.0:
            insights.append(
                f"💰 Cost efficiency is excellent at ${metrics.cost_per_story_point:.2f} per story point"
            )

        # Team insight
        if metrics.cycle_time_hours < 24:
            insights.append(
                f"🔄 Fast cycle time ({metrics.cycle_time_hours:.1f} hours) indicates efficient development"
            )

        return insights

    def _generate_recommendations(
        self, metrics: SprintMetrics, alerts: list[dict]
    ) -> list[str]:
        ...
        if metrics.parallel_execution_rate < 60:
            recommendations.append(
                "Improve task decomposition for better parallelization"
            )
        ...
        if metrics.team_satisfaction > 8.0:
            recommendations.append(
                "Continue current team practices - satisfaction is high"
            )
    ...
    def _calculate_health_score(self, metrics: SprintMetrics) -> dict[str, Any]:
        ...
        cost_score = max(
            0, 100 - (metrics.cost_per_story_point - 5) * 10
