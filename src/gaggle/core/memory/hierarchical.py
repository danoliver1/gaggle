...
class HierarchicalMemory:
    ...
    def __init__(
        self, agent_id: str, max_items_per_level: dict[MemoryLevel, int] | None = None
    ):
    ...
    def retrieve(
        ...
        avg_relevance = (
            sum(score for _, score in sorted_candidates[:limit])
            / len(sorted_candidates[:limit])
        ...
        self.logger.debug(
            f"Retrieved {len(top_items)} items for query in {retrieval_time:.3f}s"
        )
        return result
    ...
    def promote_item(self, item_id: str, target_level: MemoryLevel) -> bool:
        ...
        self.logger.info(
            f"Promoted item {item_id} from {source_level.value} to {target_level.value}"
        )
        return True
    ...
    def cleanup_expired(self) -> dict[MemoryLevel, int]:
        ...
        for level, storage in self.memory_levels.items():
            ...
            if expired_ids:
                ...
                self.logger.info(
                    f"Removed {len(expired_ids)} expired items from {level.value}"
                )
    ...
    def get_memory_stats(self) -> dict[str, Any]:
        ...
        for level, storage in self.memory_levels.items():
            ...
            if storage:
                ...
                level_stats["avg_access_count"] = sum(access_counts) / len(
                    access_counts
                )
    ...
    def _enforce_capacity_limits(self, level: MemoryLevel) -> None:
        ...
        if len(storage) >= max_items:
            ...
            for item, _ in scored_items[:num_to_evict]:
                ...
                self.logger.debug(
                    f"Evicted item {item.id} from {level.value} due to capacity"
                )
    ...
    def _sort_by_strategy(
        self, candidates: list[tuple[MemoryItem, float]], strategy: RetrievalStrategy
...
class MemoryManager:
    ...
    def search_across_agents(
        ...
        for agent_id in search_agents:
            if agent_id in self.agent_memories:
                ...
                result = memory.retrieve(
                    query_terms, strategy=strategy, limit=limit_per_agent
                )
                if result.items:
        ...
        if include_shared:
            shared_result = self.shared_memory.retrieve(
                query_terms, strategy=strategy, limit=limit_per_agent
            )
    ...
    def get_system_memory_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "system_totals": {"total_items": 0, "total_size_estimate": 0},
        }
        ...
        for agent_id, memory in self.agent_memories.items():
            ...
            stats["system_totals"]["total_size_estimate"] += agent_stats[
                "total_size_estimate"
            ]
        ...
        stats["system_totals"]["total_size_estimate"] += shared_stats[
            "total_size_estimate"
        ]
