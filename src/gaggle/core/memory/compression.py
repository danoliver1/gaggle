...
class ContextCompressor(ABC):
    ...
    @abstractmethod
    def decompress(
        self, compressed_content: str, context: dict[str, Any] = None
    ) -> str:
...
class SummaryCompressor(ContextCompressor):
    ...
    def __init__(self, max_summary_ratio: float = 0.3):
        self.max_summary_ratio = (
            max_summary_ratio  # Max summary size as ratio of original
        )
    ...
    def compress(
        ...
        if key_sentences:
            summary_parts.append(
                "Key Points: " + " ".join(key_sentences[:5])
            )  # Top 5 sentences
    ...
    def _extract_metrics(self, text: str) -> str:
        ...
        numbers_with_units = re.findall(
            r"\b\d+(?:\.\d+)?\s*(?:ms|seconds?|minutes?|hours?|days?|MB|GB|KB)", text
        )
    ...
    def _estimate_quality(self, original: str, compressed: str) -> float:
        ...
        if original_words:
            word_preservation = len(
                compressed_words.intersection(original_words)
            ) / len(original_words)
        else:
...
class TemplateCompressor(ContextCompressor):
    ...
    def compress(
        ...
        for pattern, occurrences in patterns.items():
            if (
                occurrences >= 2 and len(pattern) > 20
            ):  # Only template significant patterns
...
class HierarchicalCompressor:
    ...
    def compress_memory_item(
        self, item: MemoryItem, strategy: str = "auto"
    ) -> CompressionResult:
    ...
    def compress_context_batch(
        self, items: list[MemoryItem], max_total_size: int = 2000
    ) -> CompressionResult:
        ...
        return CompressionResult(
            ...
            compression_ratio=(
                final_size / original_total if original_total > 0 else 1.0
            ),
    ...
    def get_compression_stats(self) -> dict[str, Any]:
        ...
        if self.stats["total_compressions"] > 0:
            avg_ratio = (
                self.stats["total_tokens_saved"] / self.stats["total_original_size"]
            )
    ...
    def _update_stats(self, result: CompressionResult) -> None:
        ...
        if method_stats["count"] > 0:
            avg_savings_per_compression = (
                method_stats["total_savings"] / method_stats["count"]
            )
