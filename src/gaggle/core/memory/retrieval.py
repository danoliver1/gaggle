...
from .hierarchical import MemoryItem, RetrievalResult, RetrievalStrategy
...
class RelevanceScore:
    ...
    def combine_scores(self, bm25_weight: float = 0.4, semantic_weight: float = 0.3, 
                      temporal_weight: float = 0.15, frequency_weight: float = 0.1, 
                      importance_weight: float = 0.05) -> float:
...
class ContextRetriever(ABC):
    ...
    @abstractmethod
    def retrieve(self, query: str, items: List[MemoryItem], limit: int = 10) -> List[Tuple[MemoryItem, RelevanceScore]]:
...
class BM25Retriever(ContextRetriever):
    ...
    def _calculate_bm25_score(self, query_terms: List[str], item_id: str) -> float:
        ...
        for term in query_terms:
            ...
            denominator = tf + self.k1 * (1 - self.b + self.b * (item_length / self.average_length))
...
class EmbeddingRetriever(ContextRetriever):
    ...
    def retrieve(self, query: str, items: List[MemoryItem], limit: int = 10) -> List[Tuple[MemoryItem, RelevanceScore]]:
        ...
        for item in items:
            ...
            similarity = self._cosine_similarity(query_embedding, self.item_embeddings[item.id])
...
class HybridRetriever(ContextRetriever):
    ...
    def retrieve(self, query: str, items: List[MemoryItem], limit: int = 10) -> List[Tuple[MemoryItem, RelevanceScore]]:
        ...
        bm25_results = self.bm25_retriever.retrieve(query, items, limit * 2)  # Get more candidates
...
class RelevanceScorer:
    ...
    def score_item(self, item: MemoryItem, query_terms: Set[str], base_score: float = 0.0) -> RelevanceScore:
    ...
    def _calculate_frequency_score(self, item: MemoryItem) -> float:
        ...
        frequency_score = math.log(1 + item.access_count) / math.log(11)  # Scale to [0, 1] range
        return min(frequency_score, 1.0)
    
    def rank_items(self, items_and_scores: List[Tuple[MemoryItem, RelevanceScore]], 
                   query_terms: Set[str]) -> List[Tuple[MemoryItem, RelevanceScore]]:
...
class AdvancedRetriever:
    ...
    def retrieve_with_fallback(
        ...
        if cache_key in self.query_cache:
            ...
            filtered_results = [
                (item, score) for item, score in cached_results 
    ...
    def _cache_results(self, cache_key: str, results: List[Tuple[MemoryItem, RelevanceScore]]) -> None:
    ...
    def get_retrieval_stats(self) -> Dict[str, Any]:
        ...
        cache_total = self.retrieval_stats["cache_hits"] + self.retrieval_stats["cache_misses"]
        cache_hit_rate = self.retrieval_stats["cache_hits"] / cache_total if cache_total > 0 else 0.0
