...
class QualityMetrics:
    ...
    def overall_quality_score(self) -> float:
        ...
        performance_score = max(0, 100 - (self.response_time_p95 / 10))  # Assume 1000ms = 0 points
...
class ReviewCriteria:
    ...
    def evaluate(self, metrics: QualityMetrics) -> tuple[bool, List[str]]:
        ...
        if metrics.code_coverage < self.min_code_coverage:
            issues.append(f"Code coverage {metrics.code_coverage:.1f}% below required {self.min_code_coverage:.1f}%")
            
        if metrics.cyclomatic_complexity > self.max_cyclomatic_complexity:
            issues.append(f"Complexity {metrics.cyclomatic_complexity:.1f} exceeds limit {self.max_cyclomatic_complexity}")
            
        if metrics.security_vulnerabilities > self.max_security_vulnerabilities:
            issues.append(f"Security vulnerabilities: {metrics.security_vulnerabilities}")
            
        if metrics.response_time_p95 > self.max_response_time_ms:
            issues.append(f"Response time {metrics.response_time_p95:.1f}ms exceeds {self.max_response_time_ms}ms")
            
        if metrics.business_value_score < self.min_business_value_score:
            issues.append(f"Business value {metrics.business_value_score:.1f} below required {self.min_business_value_score}")
            
        if metrics.test_pass_rate < self.min_test_pass_rate:
            issues.append(f"Test pass rate {metrics.test_pass_rate:.1f}% below required {self.min_test_pass_rate:.1f}%")
...
class ParallelTester:
    ...
    async def run_tests(
        ...
        context = context or {}
        logger.info(f"Starting parallel test execution for: {[t.value for t in test_types]}")
        
        # Group tests by parallel capability
        parallel_tests = [t for t in test_types if self.test_suites[t].can_run_parallel]
        sequential_tests = [t for t in test_types if not self.test_suites[t].can_run_parallel]
        ...
        if parallel_tests:
            ...
            for test_type, result in zip(parallel_tests, parallel_results):
...
class MetricsCollector:
    ...
    def _extract_metrics_from_tests(
        ...
        if 'vulnerabilities' in security_results.get('metrics', {}):
            metrics.security_vulnerabilities = security_results['metrics']['vulnerabilities']
        
        return metrics
    
    def _merge_metrics(self, base: QualityMetrics, additional: Dict[str, Any]) -> QualityMetrics:
...
class QualityGateManager:
    ...
    def configure_standard_gates(self) -> None:
        ...
        # Requirements Review
        self.add_review_stage(ReviewStage(
        ...
        self.add_review_stage(ReviewStage(
            ...
            reviewer_roles=[AgentRole.TECH_LEAD, AgentRole.SENIOR_DEV],
        ...
        self.add_review_stage(ReviewStage(
            ...
            reviewer_roles=[AgentRole.TECH_LEAD, AgentRole.SECURITY_SPECIALIST],
        ...
        self.add_review_stage(ReviewStage(
            ...
            prerequisites=[ReviewStageType.SECURITY_REVIEW, ReviewStageType.PERFORMANCE_REVIEW]
        ))
    ...
    async def execute_quality_gates(
        ...
        try:
            ...
            for stage in self._get_execution_order():
                if stage.can_start(completed_stages):
                    ...
                    if stage.stage_type == ReviewStageType.CODE_REVIEW:
                        ...
                        test_results = await self.parallel_tester.run_tests(test_types, context)
                    elif stage.stage_type == ReviewStageType.SECURITY_REVIEW:
                        test_results = await self.parallel_tester.run_tests([TestType.SECURITY_TESTS], context)
                    
                    # Execute the review stage
                    result = await self._execute_stage(stage, task, test_results, context)
                    all_results.append(result)
                    
                    if result.status == QualityGateStatus.PASSED:
                        completed_stages.add(stage.stage_type)
                    elif result.status == QualityGateStatus.FAILED:
                        logger.warning(f"Stage {stage.name} failed: {result.issues_found}")
            ...
            if overall_status != QualityGateStatus.FAILED:
                ...
                elif any(r.status == QualityGateStatus.CONDITIONAL_PASS for r in all_results):
                    overall_status = QualityGateStatus.CONDITIONAL_PASS
    ...
    async def _execute_stage(
        ...
        elif len(issues) <= 2 and not any("critical" in issue.lower() for issue in issues):
        ...
        result = ReviewResult(
            ...
            status=status,
            reviewer=stage.reviewer_roles[0] if stage.reviewer_roles else AgentRole.TECH_LEAD,
            metrics=metrics,
            issues_found=issues,
    ...
    def _get_execution_order(self) -> List[ReviewStage]:
        ...
        while remaining_stages:
            ...
            if not ready_stages:
                ...
                ready_stages = [stage for stage in remaining_stages if not stage.prerequisites]
                if not ready_stages:
    ...
    def _generate_stage_recommendations(
        ...
        if stage.stage_type == ReviewStageType.CODE_REVIEW:
            ...
            if metrics.response_time_p95 > 1000:
                recommendations.append("Optimize performance to meet response time requirements")
        ...
        if issues:
            ...
            if len(issues) > 3:
                recommendations.append("Consider breaking down this task into smaller pieces")
    ...
    def get_quality_summary(self, task_id: str) -> Dict[str, Any]:
        ...
        overall_score = sum(r.metrics.overall_quality_score() for r in results) / len(results)
        ...
        return {
            ...
            'average_stage_duration': sum(r.duration_minutes for r in results) / len(results),
