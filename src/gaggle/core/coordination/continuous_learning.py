...
from .adaptive_planning import SprintMetrics
...
class PatternRecognizer:
    ...
    def _group_events_by_similarity(self) -> dict[str, list[CoordinationEvent]]:
        ...
        for event in self.coordination_events:
            ...
            agents_key = "_".join(
                sorted([role.value for role in event.agents_involved])
            )
            context_key = "_".join(
                sorted([f"{k}:{v}" for k, v in event.context.items()])
            )
    ...
    async def _extract_pattern(
        self, group_key: str, events: list[CoordinationEvent]
        ...
        recommendations = self._generate_recommendations(
            most_common_type, avg_success, events
        )
    ...
    def _extract_preconditions(self, events: list[CoordinationEvent]) -> dict[str, Any]:
        ...
        for key in all_keys:
            values = [
                event.context.get(key) for event in events if key in event.context
            ]
    ...
    def _classify_pattern_type(
        self, event_type: str, success_rate: float
    ) -> PatternType:
        """Classify the type of pattern based on characteristics."""
        if "handoff" in event_type.lower():
            return (
                PatternType.SUCCESSFUL_HANDOFF
                if success_rate > 0.7
                else PatternType.FAILED_COORDINATION
            )
        elif "breakdown" in event_type.lower():
            return (
                PatternType.EFFECTIVE_BREAKDOWN
                if success_rate > 0.7
                else PatternType.FAILED_COORDINATION
            )
            ...
            return (
                PatternType.QUALITY_ISSUE
                if success_rate < 0.5
                else PatternType.SUCCESSFUL_HANDOFF
            )
        else:
            return (
                PatternType.SUCCESSFUL_HANDOFF
    ...
    def _generate_recommendations(
        self, event_type: str, success_rate: float, events: list[CoordinationEvent]
    ) -> list[str]:
        ...
        for agent, scores in agent_performance.items():
            ...
            if avg_score < 0.6:
                recommendations.append(
                    f"Provide additional support for {agent.value} in {event_type}"
                )
...
class PerformanceTracker:
    ...
    def record_agent_performance(
        self, agent: AgentRole, sprint_id: str, metrics: dict[str, float]
    ) -> None:
    ...
    def get_agent_trends(
        self, agent: AgentRole, lookback_sprints: int = 5
    ) -> dict[str, Any]:
        ...
        return {
            "trend": (
                "improving"
                if sum(1 for t in trends.values() if t == "improving") > len(trends) / 2
                else "stable"
            ),
...
class LearningEngine:
    ...
    async def process_sprint_completion(
        ...
        logger.info(
            f"Processing sprint completion for learning: {sprint_metrics.sprint_id}"
        )
        ...
        self.retrospective_analyzer.add_retrospective_data(
            sprint_metrics.sprint_id, retrospective_data
        )
    ...
    def _pattern_to_insight(
        self, pattern: CoordinationPattern
