...
class CapacityPlanner:
    ...
    def balance_workload(
        ...
        capacity_used: Dict[AgentRole, float] = {role: 0.0 for role in AgentRole}
        
        # Sort tasks by priority and effort
        sorted_tasks = sorted(
            tasks, 
            key=lambda t: (t.priority.value if t.priority else 3, t.estimated_effort),
            reverse=True
        )
        ...
        for task in sorted_tasks:
            ...
            for role in suitable_roles:
                ...
                if remaining_capacity >= task.estimated_effort and utilization < min_utilization:
                    best_role = role
                    min_utilization = utilization
            
            if best_role:
                assignments[best_role].append(task)
                capacity_used[best_role] += task.estimated_effort
            else:
                ...
                if suitable_roles:
                    ...
                    capacity_used[fallback_role] += task.estimated_effort
    ...
    def _get_suitable_roles(self, task: Task) -> List[AgentRole]:
        ...
        role_mapping = {
            ...
            'fullstack': [AgentRole.FULLSTACK_DEV, AgentRole.FRONTEND_DEV, AgentRole.BACKEND_DEV],
...
class AdaptiveSprintPlanner:
    ...
    async def _assess_sprint_risks(
        ...
        if complex_stories:
            assessment.risk_factors.append(RiskFactor(
                ...
                level=RiskLevel.MEDIUM if len(complex_stories) <= 2 else RiskLevel.HIGH,
                impact_score=0.7,
    ...
    def _select_sprint_stories(
        ...
        sorted_stories = sorted(
            backlog, 
            key=lambda s: (s.priority.value if s.priority else 3, -(s.story_points or 0))
        )
    ...
    async def _generate_tasks_for_story(self, story: UserStory) -> List[Task]:
        ...
        base_tasks = [
            Task(
                ...
                story_id=story.id,
                estimated_effort=max(1, (story.story_points or 3) // 4),
            ...
            Task(
                ...
                story_id=story.id,
                estimated_effort=max(2, (story.story_points or 3) // 2),
                status=TaskStatus.TODO,
                priority=story.priority
            ),
            Task(
