    ...
    def get_destinations(self, message: AgentMessage) -> List[AgentRole]:
        ...
        if message.recipient:
            ...
            for route in self.routes.values():
                if (route.message_type == message.message_type and
                    route.source == message.sender):
...
class MessageBus:
    ...
    def __init__(self, max_queue_size: int = 1000):
        ...
        self.message_queues: Dict[AgentRole, deque] = defaultdict(lambda: deque(maxlen=max_queue_size))
    ...
    def register_handler(self, handler: MessageHandler) -> None:
        ...
        self.logger.info(f"Registered handler {handler.handler_id} for {handler.agent_role.value}")
            ...
            if handler.handler_id == handler_id:
                ...
                for message_type in handler.message_types:
                    self.subscribers[message_type] = [
                        h for h in self.subscribers[message_type] 
    ...
    async def send_message(self, message: AgentMessage) -> ValidationResult:
        ...
        self.logger.debug(f"Message {message.id} queued for {len(destinations)} destinations")
        return validation
    ...
    async def _deliver_message(self, message: AgentMessage, destination: AgentRole) -> None:
        ...
        for handler in handlers:
            if handler.can_handle(message):
                try:
                    ...
                    self.logger.debug(f"Message {message.id} delivered to {handler.handler_id}")
                    
                except Exception as e:
                    self.logger.error(f"Handler {handler.handler_id} failed to process message {message.id}: {e}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics.update({
            ...
            "queue_sizes": {role.value: len(queue) for role, queue in self.message_queues.items()},
