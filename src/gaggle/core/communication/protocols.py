...
class CommunicationProtocol(ABC):
    ...
    def add_message(self, message: AgentMessage) -> ValidationResult:
        ...
        if validation.is_valid:
            ...
            self.logger.info(f"Message added to protocol {self.protocol_id}: {message.id}")
        else:
            self.logger.warning(f"Invalid message for protocol {self.protocol_id}: {validation.errors}")
    ...
    def can_accept_message(self, message: AgentMessage) -> bool:
        ...
        return (self.state not in [ProtocolState.COMPLETED, ProtocolState.FAILED, ProtocolState.CANCELLED] and
                message.message_type in self.get_expected_message_types())
...
class SprintPlanningProtocol(CommunicationProtocol):
    ...
    def validate_message_sequence(self, message: AgentMessage) -> ValidationResult:
        ...
        if not self.messages:
            if message.sender != AgentRole.PRODUCT_OWNER:
...
class CodeReviewProtocol(CommunicationProtocol):
    ...
    def validate_message_sequence(self, message: AgentMessage) -> ValidationResult:
        ...
        if message.message_type == MessageType.CODE_REVIEW:
            if message.sender not in [AgentRole.TECH_LEAD, AgentRole.QA_ENGINEER]:
                result.add_warning("Code reviews typically done by Tech Lead or QA Engineer")
    ...
    def get_next_expected_actions(self) -> List[str]:
        ...
        if last_message.message_type == MessageType.CODE_REVIEW:
            return ["Address review feedback" if not getattr(last_message, 'approved', True) 
...
class ProtocolValidator:
    ...
    protocol_registry: Dict[MessageType, Type[CommunicationProtocol]] = field(default_factory=dict)
    logger: logging.Logger = field(default_factory=lambda: logging.getLogger("protocol.validator"))
    ...
    def validate_message(self, message: AgentMessage) -> ValidationResult:
        ...
        if protocol:
            ...
            result.add_warning(f"No protocol handler for message type {message.message_type.value}")
        
        return result
    
    def _find_or_create_protocol(self, message: AgentMessage) -> Optional[CommunicationProtocol]:
        ...
        if message.correlation_id:
            for protocol in self.active_protocols.values():
                if any(msg.correlation_id == message.correlation_id for msg in protocol.messages):
                    return protocol
    ...
    def cleanup_completed_protocols(self) -> int:
        ...
        completed_protocols = [
            protocol_id for protocol_id, protocol in self.active_protocols.items()
