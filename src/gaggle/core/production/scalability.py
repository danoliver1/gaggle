...
from ...models.core import Sprint, Task, UserStory, AgentRole, Team
from ...models.enums import TaskStatus, TaskPriority, SprintStatus
from ..coordination.adaptive_planning import AdaptiveSprintPlanner, SprintMetrics
...
class SprintCluster:
    ...
    # Resource allocation
    allocated_agents: Dict[AgentRole, List[AgentInstance]] = field(default_factory=lambda: defaultdict(list))
    ...
    min_agents_per_role: Dict[AgentRole, int] = field(default_factory=lambda: defaultdict(lambda: 1))
    max_agents_per_role: Dict[AgentRole, int] = field(default_factory=lambda: defaultdict(lambda: 10))
...
class ResourceManager:
    ...
    def allocate_resources(
        self, 
        sprint_id: str, 
        requested_agents: Dict[AgentRole, int]
        ...
        for role, count in requested_agents.items():
            available_agents = [
                agent for agent in self.agent_pool[role]
        ...
        logger.info(f"Allocated {sum(len(agents) for agents in allocated.values())} agents to sprint {sprint_id}")
        return dict(allocated)
    
    def _create_agent_instances(self, role: AgentRole, count: int) -> List[AgentInstance]:
    ...
    def release_resources(self, sprint_id: str) -> None:
        ...
        logger.info(f"Released {len(released_instances)} agent instances from sprint {sprint_id}")
...
class LoadBalancer:
    ...
    def select_agent(
        ...
        if self.strategy == LoadBalancingStrategy.LEAST_LOADED:
            ...
            task_type = task_context.get('task_type', '') if task_context else ''
...
class ScalabilityManager:
    ...
    async def evaluate_scaling_needs(self, cluster: SprintCluster) -> List[Dict[str, Any]]:
        ...
        for role, agents in cluster.allocated_agents.items():
            ...
            # Check for scale up
            if (utilization > 80 and 
                agent_count < cluster.max_agents_per_role[role] and
                self._can_scale(cluster.cluster_id, role)):
                ...
                scaling_actions.append({
                    ...
                    'target_count': min(agent_count + 1, cluster.max_agents_per_role[role]),
                    'reason': f'High utilization: {utilization:.1f}%'
                })
            
            # Check for scale down
            elif (utilization < 30 and 
                  agent_count > cluster.min_agents_per_role[role] and
                  self._can_scale(cluster.cluster_id, role)):
                ...
                scaling_actions.append({
                    ...
                    'role': role,
                    'current_count': agent_count,
                    'target_count': max(agent_count - 1, cluster.min_agents_per_role[role]),
    ...
    async def execute_scaling_action(self, action: Dict[str, Any]) -> bool:
        ...
        try:
            if action['action'] == 'scale_up':
                ...
                new_instances = self.resource_manager._create_agent_instances(role, needed)
                self.resource_manager.agent_pool[role].extend(new_instances)
                
                logger.info(f"Scaled up {role.value} in cluster {cluster_id}: {current_count} -> {target_count}")
                ...
                logger.info(f"Scaled down {role.value} in cluster {cluster_id}: {current_count} -> {target_count}")
    ...
    def get_scaling_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': successful_ops / len(recent_operations) * 100 if recent_operations else 0,
            'scale_up_count': len([op for op in recent_operations if op['action'] == 'scale_up']),
            'scale_down_count': len([op for op in recent_operations if op['action'] == 'scale_down']),
...
class SprintOrchestrator:
    ...
    async def create_sprint_cluster(
        ...
        if 'resource_limits' in cluster_config:
            for resource_type_str, limit_config in cluster_config['resource_limits'].items():
    ...
    def _select_best_cluster(self, sprint: Sprint) -> str:
        ...
        best_cluster_id = min(
            ...
            key=lambda cid: sum(
                self.active_clusters[cid].get_utilization(role) 
    ...
    def _calculate_required_agents(self, sprint: Sprint) -> Dict[AgentRole, int]:
        ...
        story_count = len(sprint.stories) if sprint.stories else 3
    ...
    async def manage_cluster_scaling(self) -> Dict[str, Any]:
        ...
        for cluster in self.active_clusters.values():
            if cluster.auto_scaling_enabled:
                ...
                scaling_actions = await self.scalability_manager.evaluate_scaling_needs(cluster)
                
                # Execute scaling actions
                for action in scaling_actions:
                    success = await self.scalability_manager.execute_scaling_action(action)
                    if success:
                        scaling_summary['scaling_actions_taken'] += 1
                    scaling_summary['actions'].append({
                        **action,
                        'success': success
                    })
    ...
    async def get_orchestration_status(self) -> Dict[str, Any]:
        ...
        utilization = self.scalability_manager.resource_manager.get_resource_utilization()
        ...
        for cluster_id, cluster in self.active_clusters.items():
            cluster_details[cluster_id] = {
                ...
                'utilization_by_role': {
                    role.value: cluster.get_utilization(role)
                    for role in AgentRole
                }
            }
