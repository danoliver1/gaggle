...
from ..coordination.adaptive_planning import SprintMetrics
...
class MetricThreshold:
    ...
    def evaluate(self, value: float, recent_values: List[float] = None) -> AlertSeverity:
...
class AlertRule:
    ...
    def matches_metric(self, metric: HealthMetric) -> bool:
        ...
        return self.metric_pattern in metric.name or metric.name.startswith(self.metric_pattern)
...
class SprintHealthMonitor:
    ...
    async def _check_rule_thresholds(self, rule: AlertRule, metric: HealthMetric) -> None:
    ...
    async def _trigger_alert(
        ...
        for alert in self.active_alerts.values():
            if (alert.metric_name == metric.name and 
                alert.sprint_id == metric.sprint_id and
                not alert.resolved and
                alert.age_minutes() < rule.cooldown_minutes):
        ...
        if existing_alert:
            ...
            existing_alert.severity = max(existing_alert.severity, severity, key=lambda x: x.value)
        else:
            # Create new alert
            alert = Alert(
                alert_id=alert_id,
                ...
                recommended_actions=self._generate_alert_actions(rule, metric, severity)
            )
    ...
    async def _execute_auto_action(self, action: str, alert: Alert) -> None:
        ...
        alert.auto_actions_taken.append({
            'action': action,
    ...
    def get_sprint_health(self, sprint_id: str) -> Dict[str, Any]:
        ...
        for metric_type, metrics_queue in self.metrics.items():
            sprint_metrics.extend([
                ...
                if (metric.sprint_id == sprint_id and 
        ...
        velocity_metrics = [m for m in sprint_metrics if m.metric_type == MetricType.VELOCITY]
        current_velocity = velocity_metrics[-1].value if velocity_metrics else 0
        
        quality_metrics = [m for m in sprint_metrics if m.metric_type == MetricType.QUALITY]
        avg_quality_score = (
    ...
    def get_dashboard_data(self, dashboard_id: str, time_range_hours: int = None) -> Dict[str, Any]:
        ...
        for panel in dashboard.panels:
            ...
            for query in panel['metric_queries']:
                ...
                for metric_type, metrics_queue in self.metrics.items():
                    matching_metrics = [
                        metric for metric in metrics_queue
                        if (query in metric.name or query.replace('*', '') in metric.name) and
                           metric.timestamp >= cutoff_time
                    ]
                    panel_metrics.extend(matching_metrics)
            
            # Process metrics based on panel type
            processed_data = self._process_panel_data(panel['panel_type'], panel_metrics)
    ...
    def get_monitoring_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_metric_received': max(
                (max(queue, key=lambda m: m.timestamp).timestamp for queue in self.metrics.values() if queue),
                default=None
            )
        }