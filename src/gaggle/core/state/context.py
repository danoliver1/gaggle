...
class AgentContext:
    ...
    def get_context(self, item_id: str, level: Optional[ContextLevel] = None) -> Optional[ContextItem]:
    ...
    def search_context(self, query: str, level: Optional[ContextLevel] = None, limit: int = 10) -> List[ContextItem]:
...
class ContextManager:
    ...
    def get_or_create_context(self, agent_role: AgentRole, agent_id: str) -> AgentContext:
        ...
        if agent_id not in self.agent_contexts:
            ...
            self.logger.info(f"Created context for agent {agent_id} ({agent_role.value})")
    ...
    def add_shared_context(self, item: ContextItem) -> None:
        ...
        self.logger.debug(f"Added shared context item {item.id} at level {item.level.value}")
    
    def get_shared_context(self, level: ContextLevel, item_id: str) -> Optional[ContextItem]:
    ...
    def search_shared_context(self, query: str, level: Optional[ContextLevel] = None, limit: int = 10) -> List[ContextItem]:
    ...
    def update_agent_state(self, agent_id: str, new_state: str) -> bool:
        ...
        if agent_id in self.agent_contexts:
            ...
            self.logger.info(f"Agent {agent_id} state changed: {old_state} -> {new_state}")
            return True
        return False
    ...
    def get_system_context_summary(self) -> Dict[str, Any]:
        ...
        for level in ContextLevel:
            summary["shared_context_counts"][level.value] = len(self.shared_context[level])
