...
class StateTransition:
    ...
    condition: Optional[Callable[[Any], bool]] = None  # Additional condition check
    action: Optional[Callable[[Any], Any]] = None  # Action to perform during transition
...
class StateMachineConfig:
    ...
    state_timeouts: Dict[AgentState, float] = field(default_factory=dict)  # Timeout in seconds
...
class AgentStateMachine(ABC):
    ...
    def __init__(self, agent_role: AgentRole, agent_id: str, config: Optional[StateMachineConfig] = None):
        ...
        self.current_message: Optional[AgentMessage] = None
    ...
    def transition_to(self, new_state: AgentState, trigger: str = "manual", context: Any = None) -> bool:
        """Attempt to transition to a new state."""
        
        if new_state not in self.config.valid_states:
            self.logger.warning(f"Invalid state transition attempted: {self.current_state} -> {new_state}")
            return False
        
        # Find applicable transition
        applicable_transitions = [
            t for t in self.config.get_transitions_from_state(self.current_state)
            if t.to_state == new_state and t.trigger == trigger
        ]
        
        if not applicable_transitions:
            self.logger.warning(f"No valid transition found: {self.current_state} -> {new_state} (trigger: {trigger})")
            return False
        ...
        if not transition.can_transition(context):
            self.logger.warning(f"Transition condition failed: {self.current_state} -> {new_state}")
            return False
...
class ProductOwnerStateMachine(AgentStateMachine):
    ...
    def _get_default_config(self) -> StateMachineConfig:
        ...
        # Define transitions
        transitions = [
            StateTransition(AgentState.IDLE, AgentState.PLANNING, "sprint_planning_started"),
            StateTransition(AgentState.IDLE, AgentState.COORDINATING, "requirement_clarification_requested"),
            StateTransition(AgentState.PLANNING, AgentState.COORDINATING, "stakeholder_input_needed"),
            StateTransition(AgentState.PLANNING, AgentState.IDLE, "planning_completed"),
            StateTransition(AgentState.COORDINATING, AgentState.PLANNING, "requirements_clarified"),
            StateTransition(AgentState.COORDINATING, AgentState.REVIEWING, "sprint_review_started"),
...
class ScrumMasterStateMachine(AgentStateMachine):
    ...
    def _get_default_config(self) -> StateMachineConfig:
        ...
        transitions = [
            StateTransition(AgentState.IDLE, AgentState.PLANNING, "sprint_planning_initiated"),
            StateTransition(AgentState.IDLE, AgentState.COORDINATING, "daily_standup_time"),
            StateTransition(AgentState.PLANNING, AgentState.IDLE, "sprint_planned"),
            StateTransition(AgentState.COORDINATING, AgentState.IDLE, "coordination_completed"),
            StateTransition(AgentState.COORDINATING, AgentState.REVIEWING, "retrospective_time"),
            StateTransition(AgentState.REVIEWING, AgentState.IDLE, "retrospective_completed"),
            # Blocked transitions
            StateTransition(AgentState.IDLE, AgentState.BLOCKED, "blocked"),
            StateTransition(AgentState.PLANNING, AgentState.BLOCKED, "blocked"),
            StateTransition(AgentState.COORDINATING, AgentState.BLOCKED, "blocked"),
...
class TechLeadStateMachine(AgentStateMachine):
    ...
    def _get_default_config(self) -> StateMachineConfig:
        ...
        transitions = [
            StateTransition(AgentState.IDLE, AgentState.PLANNING, "architecture_planning_needed"),
            StateTransition(AgentState.IDLE, AgentState.REVIEWING, "code_review_requested"),
            StateTransition(AgentState.IDLE, AgentState.WORKING, "architecture_task_assigned"),
            StateTransition(AgentState.PLANNING, AgentState.WORKING, "architecture_decided"),
            StateTransition(AgentState.PLANNING, AgentState.IDLE, "planning_completed"),
            StateTransition(AgentState.WORKING, AgentState.REVIEWING, "architecture_completed"),
            StateTransition(AgentState.WORKING, AgentState.IDLE, "task_completed"),
            StateTransition(AgentState.REVIEWING, AgentState.IDLE, "review_completed"),
            StateTransition(AgentState.REVIEWING, AgentState.COORDINATING, "review_feedback_needed"),
            StateTransition(AgentState.COORDINATING, AgentState.IDLE, "coordination_completed"),
            # Blocked transitions
            StateTransition(AgentState.IDLE, AgentState.BLOCKED, "blocked"),
            StateTransition(AgentState.PLANNING, AgentState.BLOCKED, "blocked"),
...
class DeveloperStateMachine(AgentStateMachine):
    ...
    def _get_default_config(self) -> StateMachineConfig:
        ...
        transitions = [
            ...
            StateTransition(AgentState.IDLE, AgentState.COORDINATING, "clarification_needed"),
            StateTransition(AgentState.WORKING, AgentState.COORDINATING, "help_needed"),
            StateTransition(AgentState.WORKING, AgentState.IDLE, "task_completed"),
            StateTransition(AgentState.COORDINATING, AgentState.WORKING, "clarification_received"),
            StateTransition(AgentState.COORDINATING, AgentState.IDLE, "coordination_completed"),
            # Blocked transitions
            StateTransition(AgentState.IDLE, AgentState.BLOCKED, "blocked"),
            StateTransition(AgentState.WORKING, AgentState.BLOCKED, "blocked"),
...
class QAEngineerStateMachine(AgentStateMachine):
    ...
    def _get_default_config(self) -> StateMachineConfig:
        ...
        transitions = [
            StateTransition(AgentState.IDLE, AgentState.PLANNING, "test_planning_needed"),
            StateTransition(AgentState.IDLE, AgentState.WORKING, "testing_task_assigned"),
            StateTransition(AgentState.IDLE, AgentState.REVIEWING, "code_review_requested"),
            StateTransition(AgentState.PLANNING, AgentState.WORKING, "test_plan_completed"),
