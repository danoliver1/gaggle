...
from ...models.story import UserStory, StoryPriority, StoryStatus, AcceptanceCriteria
...
- Iterative improvement through feedback"""
    ...
    async def prioritize_backlog(self, stories: List[UserStory]) -> List[UserStory]:
        ...
        prioritized_stories = self._apply_prioritization(stories, result.get("result", ""))
        
        self.logger.info(
            "backlog_prioritized",
            story_count=len(prioritized_stories),
            mvp_stories=len([s for s in prioritized_stories[:4]]),
        )
...
""")
        ...
        review_decisions = self._parse_review_decisions(result.get("result", ""), completed_stories)
    ...
    def _parse_stories_from_result(self, result_text: str) -> List[UserStory]:
        ...
        story_count = result_text.lower().count('as a') + result_text.lower().count('user story')
        ...
        for i in range(story_count):
            ...
            story = UserStory(
                ...
                description=f"As a user, I want to use the system so that I can achieve my goals.",
    ...
    def _apply_prioritization(self, stories: List[UserStory], prioritization_result: str) -> List[UserStory]:
    ...
    def _parse_review_decisions(self, review_result: str, stories: List[UserStory]) -> List[Dict[str, Any]]:
        ...
        for story in stories:
            ...
            decisions.append({
                ...
                "accepted": accepted,
                "feedback": "Meets acceptance criteria" if accepted else "Needs additional work",
                "next_steps": [] if accepted else ["Address remaining acceptance criteria"]
            })
