...
from ...tools.project_tools import BlockerTracker, MetricsTool, SprintBoardTool
...
- Focus on value delivery"""
    ...
    async def facilitate_daily_standup(
        self, sprint: SprintModel, team_config: TeamConfiguration
    ) -> dict[str, Any]:
        """Facilitate daily standup meeting."""

        sprint_report = sprint.get_daily_standup_report()
        team_config.get_team_workload()
        ...
        **Recent Updates:**
        ...
        standup_output = {
            ...
            "sprint_goal_status": (
                "on_track" if sprint_report["completion_percentage"] > 60 else "at_risk"
            ),
        }
    ...
    async def conduct_sprint_retrospective(
        ...
        **Sprint Notes:**
        ...
        retro_output = {
            ...
            "areas_for_improvement": self._extract_improvements(
                result.get("result", "")
            ),
            "action_items": self._extract_action_items(result.get("result", "")),
            ...
            "recommendations_next_sprint": self._generate_recommendations(
                result.get("result", "")
            ),
        }
    ...
    async def track_sprint_metrics(self, sprint: SprintModel) -> dict[str, Any]:
        ...
        **Sprint {sprint.id} Metrics:**
        ...
        return {
            ...
            "velocity_status": (
                "on_track" if sprint.metrics.completion_percentage > 70 else "at_risk"
            ),
    ...
    def _select_stories_for_sprint(
        self, stories: list[UserStory], planning_result: str
    ) -> list[str]:
    ...
    def _estimate_velocity(
        self, stories: list[UserStory], team_capacity: dict
    ) -> float:
