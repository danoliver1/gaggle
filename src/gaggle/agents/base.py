...
from ..core.communication import AgentMessage, MessageBus, MessageHandler
from ..core.state import AgentStateMachine, ContextManager
...
class AgentContext:
    ...
    def increment_tokens(
        self, input_tokens: int, output_tokens: int, role: AgentRole
    ) -> None:
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.agent_context = self.context_manager.get_or_create_context(
            role, self.agent_id
        )

        # State machine for context-aware coordination
        self.state_machine: AgentStateMachine | None = self._create_state_machine()
    ...
    def _create_state_machine(self) -> AgentStateMachine | None:
