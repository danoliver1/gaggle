...
from ...tools.review_tools import ArchitectureReviewTool, CodeReviewTool
from ..base import AgentContext, ArchitectureAgent
...
- Team productivity through good patterns"""
    ...
    async def analyze_technical_complexity(
        ...
        self.logger.info(
            ...
            avg_complexity=sum(
                self._complexity_to_score(cs["complexity_rating"])
                for cs in complexity_scores.values()
            )
            / len(complexity_scores),
            high_complexity_stories=len(
                [
                    cs
                    for cs in complexity_scores.values()
        ...
        return {
            ...
            "recommendations": self._generate_technical_recommendations(
                complexity_scores, technical_risks
            ),
        }

    async def break_down_into_tasks(
        self, stories: list[UserStory]
    ) -> dict[str, list[Task]]:
        ...
        self.logger.info(
            ...
            parallelization_percentage=(
                (parallelizable_tasks / total_tasks * 100) if total_tasks > 0 else 0
            ),
        )

        return tasks_by_story

    async def generate_reusable_components(
        self, stories: list[UserStory]
    ) -> dict[str, Any]:
        """Generate reusable components that can save tokens during the sprint."""

        stories_context = "\n".join(
            [f"- {story.title}: {story.description[:100]}..." for story in stories]
        )
    ...
    async def identify_parallel_execution_plan(
        ...
        **All Sprint Tasks:**
        ...
        execution_plan = self._create_execution_plan(
            all_tasks, result.get("result", "")
        )

        parallelization_metrics = self._calculate_parallelization_metrics(
        ...
        self.logger.info(
            ...
            parallelization_efficiency=parallelization_metrics.get(
    ...
    async def review_code_architecture(
        self, code_files: list[str], context: str
    ) -> dict[str, Any]:
    ...
    def _assess_architecture_impact(self, story: UserStory) -> str:
        ...
        if any(
            word in description
            for word in ["architecture", "refactor", "migration", "breaking"]
        ):
            return "High"
        elif any(
            word in description for word in ["integration", "api", "schema", "service"]
        ):
            return "Medium"
        else:
            return "Low"
    ...
    def _generate_technical_recommendations(
        self, complexity_scores: dict, risks: list
    ) -> list[str]:
        ...
        if high_complexity_count > 2:
            recommendations.append(
                "Consider breaking down high-complexity stories further"
            )
    ...
    def _parse_tasks_from_breakdown(
        self, story: UserStory, breakdown_text: str
    ) -> list[Task]:
    ...
    def _parse_reusable_components(
    ...
    def _create_usage_instructions(
...
"""
    ...
    def _create_execution_plan(
        self, tasks: list[Task], planning_result: str
    ) -> dict[str, Any]:
        ...
        plan = {
            ...
            "critical_path": [
                t.id for t in tasks if t.complexity == TaskComplexity.HIGH
            ],
            "integration_points": [
                {
    ...
    def _calculate_parallelization_metrics(
        self, execution_plan: dict, tasks: list[Task]
    ) -> dict[str, Any]:
